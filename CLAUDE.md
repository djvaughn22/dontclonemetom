...
- Personal-name exception (owner decision, Jul 2026): Isaiah — DJ's family dog — is public by name with his own profile at `/dogs/isaiah`. Canonical hero identity: **Bruzer Zayne** (id `bruzer-zayne`), subtitle "The Dark Zay", real name Isaiah. "Bruce Zayne"/"Bruze Zayne"/"Bruiser" are misspellings, test-banned (`BANNED_HERO_SPELLINGS`). No other personal names on the site.
- Dog Identity Engine (Jul 2026): pure deterministic generator in `app/lib/identity/` (docs: `docs/DOG-IDENTITY-ENGINE.md`) powering the free `/legend` studio. Hidden-technology rule: never market this as AI publicly (it's deterministic code anyway). Truth boundary: the engine only riffs on owner-confirmed facts; rights labels (`playSafe`/`manualReview`/`blockedForMerch`) are honest and human review sits before any merch. Free preview is always watermarked with the Isaiah approval seal; photos never leave the visitor's browser. Commerce (`commerce.ts`) reports only truthful connection states — nothing is "connected" without a verified auth flag. Donation copy stays gated until a real charity + written permission exist. iDontCry port plan: `docs/IDONTCRY-HANDOFF.md`.
